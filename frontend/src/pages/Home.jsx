    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                ...
                console.log("fetch post: ", fetchPostData);
                console.log("date:", new Date(fetchPostData[0].createdAt));
                ...
                for (let i = 0; i < fetchPostData.length; i++) {
                    ...
                    console.log("fetch post user: ", fetchPostUserData);
        ...
        <>
            ...
            <div className={`flex bg-[url('./assets/images/cartoon-bg.png')] bg-cover bg-fixed fixed font-nunito text-white pt-[60px] ${showSidebar ? "pl-[12.5rem]" : "pl-[5.5rem]"} h-full w-full`}>
                <div className="relative flex flex-1 overflow-y-auto">
                    ...
                    <section className={`flex flex-col gap-4 justify-start left-[40%] relative w-[60%] py-8 px-4 overflow-hidden`}>
                        {posts.map((post, index) => (
                            <div key={index} className="flex justify-center w-[70%] min-h-[16rem] border-2 border-primary-pink rounded-[30px] bg-black bg-opacity-60 backdrop-blur-md p-4">
